...
- `WithAttemptTimeout(time.Duration)`: default 10 seconds, applying through request context to send and body read; it does not bound request preparation or later synchronous decoding.
- `WithRetryPolicy(RetryPolicy)`: immutable copy; `MaxRetries: 0` disables retries. Its total budget covers HTTP attempts, body reads, and backoff, not request preparation or later synchronous decoding.
...
Retry HTTP 408, 429, and all 5xx including 529 based on the known status even when gzip initialization or an ordinary error-body read fails; this status behavior is independent of connection/timeout retry flags. Retry connection errors, interrupted successful-body reads, and per-attempt timeouts according to policy. A known non-2xx status yields its safe `APIError` over ordinary body I/O or attempt-timeout failures, but caller cancellation and non-retryable `ResponseTooLargeError` retain precedence. Never retry local validation/protocol errors, body-too-large errors, redirects, or other 4xx. Parse `retry-after-ms` first, then `Retry-After` decimal seconds or HTTP date. A valid server delay takes precedence over backoff, including when the error body is malformed. If it cannot complete inside the lesser of caller deadline and total budget, return the current error immediately; never substitute a shorter delay and retry early. Invalid/missing headers use capped exponential subtractive jitter. Inject private clock/random/wait seams only as needed for deterministic tests; do not expose them.
